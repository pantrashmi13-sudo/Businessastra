...
function SalesReturnDetail() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={`Sales Return: ${r.return_number}`}
        description={`Against Invoice #${(r.sales_invoices as any)?.invoice_number || "—"}`}
        actions={
        ...
        <CardContent>
          <div className="grid grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{(r.customers as any)?.name || "—"}</p>
            </div>
            <div>
              <span className="text-muted-foreground">Subtotal</span>
              <p className="font-medium">{inr(r.subtotal)}</p>
            </div>
            <div>
